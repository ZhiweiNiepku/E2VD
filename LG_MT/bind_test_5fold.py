...
class SoluModel(nn.Module):
    def __init__(self, seq_len ,in_dim=DIM_SIZE[BACKBONE], sa_out=DIM_SIZE[BACKBONE], conv_out=DIM_SIZE[BACKBONE]):
...
class SoluModel_noLG(nn.Module):
    def __init__(self, seq_len ,in_dim=1024, sa_out=1024, conv_out=1024):
        super(SoluModel_noLG, self).__init__()
    ...
    def forward(self, feats):
        ...
        out=torch.max(feats,dim=1)[0].squeeze()
...
if __name__ == '__main__':
    ...
    #model = SoluModel_noLG(SEQ_LEN)
    
    model = model.to(device)

    if 1:
        # load train data
        train_feats=np.load('<path to data>/single_bind_{}_train_data.npy'.format(BACKBONE))
        ...
        # load test data
        test_feats=np.load('<path to data>/single_bind_{}_test_data.npy'.format(BACKBONE))
        test_reg_label=np.load('<path to data>/single_bind_{}_test_label.npy'.format(BACKBONE))
