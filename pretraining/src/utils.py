...
try:
    ...
    print("import Moxing failed. Maybe it's in local environment")
...
class BertLearningRate(LearningRateSchedule):
    ...
    def __init__(self, learning_rate, end_learning_rate, warmup_steps, decay_steps, power, init_steps=0):
        ...
        # self.first =  Tensor(np.array([1]).astype(np.int))
        self.init_steps = init_steps

    def construct(self, global_step):

        global_step = global_step + self.init_steps
        ...
        # if self.first == 1:
        #     print("The first learning rate is: " )
        #     print(lr)
        #     self.first = Tensor(np.array([0]).astype(np.int))
