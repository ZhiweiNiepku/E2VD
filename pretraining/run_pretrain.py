...
from src.dataset import create_bert_dataset, create_eval_dataset, create_txt_dataset
...
def _get_optimizer(args_opt, network, decaysteps):
    """get bert optimizer, support Lamb, Momentum, AdamWeightDecay."""
    if cfg.optimizer == 'Lamb':
        lr_schedule = BertLearningRate(learning_rate=cfg.learning_rate,
                                       end_learning_rate=cfg.Lamb.end_learning_rate,
                                       warmup_steps=cfg.Lamb.warmup_steps,
                                       decay_steps=decaysteps,
                                       power=cfg.Lamb.power,
                                       init_steps=cfg.init_steps)
...
def modelarts_pre_process():
    ...
    cfg.eval_data_dir = cfg.eval_data_path
    cfg.save_checkpoint_path = os.path.join(cfg.output_path, cfg.save_checkpoint_path)

    cfg.AdamWeightDecay.learning_rate = cfg.learning_rate
    cfg.Lamb.learning_rate = cfg.learning_rate
    cfg.Momentum.learning_rate = cfg.learning_rate

    cfg.AdamWeightDecay.power = cfg.power
    cfg.Lamb.power = cfg.power

    cfg.AdamWeightDecay.warmup_steps = cfg.warmup_steps_
    cfg.Lamb.warmup_steps = cfg.warmup_steps_

    cfg.AdamWeightDecay.eps = cfg.eps
    cfg.Lamb.eps = cfg.eps

    cfg.AdamWeightDecay.weight_decay = cfg.weight_decay
    cfg.Lamb.weight_decay = cfg.weight_decay
    cfg.Thor.weight_decay = cfg.weight_decay

    bert_net_cfg.seq_length = cfg.seq_length
    bert_net_cfg.num_hidden_layers = cfg.num_hidden_layers

    cfg.AdamWeightDecay.end_learning_rate = cfg.end_learning_rate
    cfg.Lamb.end_learning_rate = cfg.end_learning_rate
...
def run_pretrain():
    ...
    print('sys.stdout redirect', sys.stdout.buffer)
    print("===config is: ", cfg, flush=True)
    ...
    optimizer = _get_optimizer(cfg, net_with_loss, total_steps)
    ...
    if cfg.train_with_eval == 'true':
        ...
        if not cfg.data_type == "txt":
            ...
            eval_ds = create_txt_dataset(device_num=device_num, rank=rank, do_shuffle="false", data_dir=cfg.eval_data_dir,
