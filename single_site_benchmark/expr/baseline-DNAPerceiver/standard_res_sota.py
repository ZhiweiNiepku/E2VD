...
class ProteinSeqDataset(Dataset):
    def __init__(self, df):
        self.seqs = df[:,0]
        self.reg_labels = torch.from_numpy(df[:,1].astype(float))
...
if __name__ == '__main__':
    ...
    res_dir = "result_{}_lr{}_regk{}_k{}_f1_prop_".format(BATCH_SIZE,lr,REG_LOSS_K,k)
    ...
    df_all=pd.read_csv('data/single_site_benchmark/expr/expr_4k_train.csv').values
    print(df_all.shape)
    
    cls_label = df_all[:,1]>=1
