...
if __name__ == '__main__':
    ...
    if 1:
        # load train data
        train_feats=np.load('<path to data>/mine_rare_train_data.npy')
        ...
        # load test data
        test_feats=np.load('<path to data>/mine_rare_test_data.npy')
        test_reg_label=np.load('<path to data>/mine_rare_test_label.npy')
        #test_feats=np.load('../data/85_train_data.npy')
        #test_reg_label=np.load('../data/85_train_label.npy')
    ...
    with torch.no_grad():
        for model_i in range(model_n):
            ...
            vc_=(test_cls_label==y_cls_bool).sum()/len(y_cls_bool)
            ...
            np.save('y_cls_{}_{}.npy'.format(thres, model_i),y_cls)
            np.save('y_reg_{}_{}.npy'.format(thres, model_i),y_reg)
            np.save('y_label_{}_{}.npy'.format(thres, model_i),test_reg_label)
