...
if __name__ == '__main__':
    ...
    if 1:
        # load train data
        train_feats=np.load('<path to data>/single_bind_customize_train_data.npy')
        train_reg_label=np.load('<path to data>/single_bind_customize_train_label.npy')
    ...
    with torch.no_grad():
        for model_i in range(model_n):
            ...
            for step, (feat_, cls_label_, reg_label_) in enumerate(test_loader):
                ...
                if device.type=='cpu':
                    output = model(feat_)#.data.numpy().squeeze()
