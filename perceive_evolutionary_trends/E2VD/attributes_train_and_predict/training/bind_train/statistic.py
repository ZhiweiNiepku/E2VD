...
# this is for the prediction in predict_bind_emb_main_pickle.py
import numpy as np


feats1=np.load('<path to training data>/bind_train_data.npy')
feats2=np.load('<path to training data>/bind_test_data.npy')
label1=np.load('<path to training data>/bind_train_label.npy').reshape(-1,1)
label2=np.load('<path to training data>/bind_test_label.npy').reshape(-1,1)
