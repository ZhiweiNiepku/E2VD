...
if __name__ == '__main__':
    ...
    df_all=pd.read_csv('data/other_virus/zika/data_train.csv').values
    print(df_all.shape)
    ...
    for i_ in range(k):
        ...
        Train_dataset = ProteinSeqDataset(df_train, scaling = 10)
        train_loader_single = DataLoader(dataset=Train_dataset, batch_size=BATCH_SIZE, shuffle=True)

        Val_dataset = ProteinSeqDataset(df_val, scaling = 10)
